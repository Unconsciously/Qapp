...
//配置页面路由----2个页面： index、citywalk
var root = '/' || '/html/qyerIndex.html';
var cityWalk = '/html/qyerCityWalk.html';

app.get(root, function (req, res) {
    var realPath = fs.realpathSync('./Qapp/html/qyerIndex.html');
    res.sendFile(realPath);
});
app.get(cityWalk, function (req, res) {
    var realPath = fs.realpathSync('./Qapp/html/qyerCityWalk.html');
    res.sendFile(realPath);
});

//配置数据接口--get---4个数据：getBanner getFreeWalk  getMenu  getCityWalkList
//要从2方面验证配置的接口是否正确：
//  1:浏览器端: http://10.3.157.189:8888/getBanner  观察是否显示数据
//  2:ajax: http://10.3.157.189:8888/  打开根目录(即测试页面test.html)，触发事件：发送ajax请求  观察是否打印数据
...
//-------'/getCityWalkList'
app.get('/getCityWalkList', function (req, res) {
    var cityWalkListData = {
        err: 1,
        msg:[
            {
    ...
    res.send(JSON.stringify(cityWalkListData));
});


app.get('*', function (req, res) {
    if (req.url != '/favicon.ico'){
        var realPath = fs.realpathSync('./Qapp' + req.url);
        res.sendFile(realPath);
    }

});

//后台配置的域名：localhost，再用ip访问则失败，只能用localhost来访问该服务
